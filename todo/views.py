    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():   
                ...
                messages.success(request, "Registration successful" + '' + user1)
...
def createTask(request):
    if request.method == "POST":
        ...
        todo=Todo(user=request.user , title=title ,description=desc , complete=status , created=date)
        todo.save()
...
def dueEmail(request):
    task=Todo.objects.filter(user=request.user , complete=True)
    for task in task:
        if task.created.date() < date.today():
            print("hell")
            mail_subject='Overdue Task'
